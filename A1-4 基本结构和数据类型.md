...
在Go中，_的作用与python代码中有点类似：

+ _ 本身就是一个特殊的标识符，被称为空白标识符。它可以像其他标识符那样用于变量的声明或赋值（任何类型都可以赋值给它）
...
包是结构化代码的一种方式：每个程序都由包（通常简称为 pkg）的概念组成，可以使用自身的包或者从其它包中导入内容。如同其它一些编程语言中的类库或命名空间的概念：
...
示例 4.1 [hello_world.go](https://learnku.com/docs/the-way-to-go/the-basic-structure-and-elements-of-the-go-program/examples/chapter_4/hello_world.go)：
...
1. 编译 `C.go`, `B.go`, 然后是 `A.go`.
2. 为了编译 `A.go`, 编译器读取的是 `B.o` 而不是 `C.o`.
...
示例 4.2 [alias.go](https://learnku.com/docs/the-way-to-go/the-basic-structure-and-elements-of-the-go-program/examples/chapter_4/alias.go)
...
+ 编译器会产生 func main() ; 这样的结果，很明显这错误的
...
示例 4.2 [hello_world2.go](https://learnku.com/docs/the-way-to-go/the-basic-structure-and-elements-of-the-go-program/examples/chapter_4/hello_world2.go)
...
**一个包可以分散在多个文件中，但是只需要在其中一个进行注释说明即可。当开发人员需要了解包的一些情况时，**

自然会用 godoc 来显示包的文档说明，在首行的简要注释之后可以用成段的注释来进行更详细的说明，而不必拥挤在一起。另外，在多段注释之间应以空行分隔加以区分。
...
示例 4.4 [gotemplate.go](https://learnku.com/docs/the-way-to-go/the-basic-structure-and-elements-of-the-go-program/examples/chapter_4/gotemplate.go)
...
干净、可读的代码和简洁性是 Go 追求的主要目标。通过 gofmt 来强制实现统一的代码风格。Go 语言中对象的命名也应该是简洁且有意义的。像 Java 和 Python 中那样使用混合着大小写和下划线的冗长的名称会严重降低代码的可读性。名称不需要指出自己所属的包，因为在调用的时候会使用包名作为限定符。
...
程序中所用到的内存在计算机中使用一堆箱子来表示（这也是人们在讲解它的时候的画法），这些箱子被称为 “字节”。根据不同的处理器以及操作系统类型，所有的字都具有 32 位（4 字节）或 64 位（8 字节）的相同长度；所有的字都使用相关的内存地址来进行表示（以十六进制数表示）。

所有像 int、float、bool 和 string 这些基本类型都属于值类型，使用这些类型的变量直接指向存在内存中的值：

<img src="./assets/0-2872058.png" alt="file"  />

**另外，像数组（第 7 章）和结构（第 10 章）这些复合类型也是值类型。**

当使用等号 `=` 将一个变量的值赋值给另一个变量时，如：`j = i`，实际上是在内存中将 i 的值进行了拷贝

![file](./assets/0-20241129172214322.png)
...
![file](./assets/0-20241129172631905.png)
...
这个格式化字符串可以含有一个或多个的格式化标识符，例如：%..，其中 .. 可以被不同类型所对应的标识符替换，如 %s 代表字符串标识符、%v 代表使用类型的默认输出格式的标识符。

这些标识符所对应的值从格式化字符串后的第一个逗号开始按照相同顺序添加，如果参数超过 1 个则同样需要使用逗号分隔。使用这些占位符可以很好地控制格式化输出的文本。
...
使用操作符 := 可以高效地创建一个新的变量，称之为**初始化声明** 。**下面是相关注意事项：**
...
1. 同一类型的多个变量可以声明在同一行(这是将类型写在标识符后面的一个重要原因)，如：
...
3. 上面这行假设了变量 a，b 和 c 都已经被声明，否则的话应该这样使用：
...
练习 4.1 [local_scope.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_4/local_scope.go):
...
练习 4.2 [global_scope.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_4/global_scope.go):
...
练习 4.3 [function_calls_function.go](https://github.com/unknwon/the-way-to-go_ZH_CN/blob/master/eBook/examples/chapter_4/function_calls_function.go)
...
Go 对于值之间的比较有**非常严格的限制** ，只有两个类型相同的值才可以进行比较：
...
##### 5.2.1 整数 int 和 浮点数 float

Go 语言支持整型和浮点型数字，并且原生支持复数，其中位的运算采用补码（详情参见 [二的补码](https://en.wikipedia.org/wiki/Two's_complement) 页面）。
...
> 在Go语言中，`int`和`int64`不是相同的类型。
>
> `int`是一个平台相关的类型，它的大小取决于操作系统的架构。在32位系统上，`int`通常是32位的，而在64位系统上，`int`通常是64位的。这意味着`int`的大小可能会因为不同的系统而不同。
>
> `int64`则总是64位的，无论在哪个操作系统上。它是一个固定大小的整数类型，提供了跨平台的一致性。
>
> 因此，尽管在某些系统上`int`和`int64`可能占用相同的存储空间，它们仍然是不同的类型，并且在类型转换时需要显式转换。例如，如果你有一个`int64`类型的变量，并且想要将其赋值给一个`int`类型的变量，你需要进行类型转换：
>
> ```go
> var int64Var int64 = 1234567890
> var intVar int = int(int64Var) // 显式类型转换
> ```

在进行跨平台编程时，通常推荐使用`int64`或其他固定大小的整数类型，以确保代码的可移植性和一致性。
...
1. 按位与 `&`：对应位置上的值经过和运算结果
2. 按位或 `|`：对应位置上的值经过或运算结果，
3. 按位异或 `^`：异或运算
4. 位清除 `&^`：将指定位置上的值设置为 0。

**一元运算符**

1. 位左移 `<<`：向左移动 n 位，右侧空白部分使用 0 填充；即 2 的 n 次方。
2. 位右移 `>>`：向右移动 n 位，左侧空白部分使用 0 填充；结果是当前值除以 2 的 n 次方。
...
> 1. `_ = iota`：这是一个特殊的行，它使用空白标识符`_`来忽略`iota`生成的第一个值。`iota`是Go中的一个常量生成器，它在每个`const`声明中被重置为0，并在每个`const`行中递增1。通过将`iota`的值赋给空白标识符，我们实际上是在跳过第一个值（0），使得`iota`从1开始计数。
>
> 2. `KB ByteSize = 1<<(10*iota)`：这是第一个实际使用的常量声明。`KB`被声明为`ByteSize`类型，其值是`1<<(10*iota)`。由于`iota`从1开始，所以`10*iota`等于10，`1<<10`等于1024，即`KB`的值是1024。这意味着`KB`代表1024字节，即1千字节（Kilobyte）。
...
**练习 4.4** 尝试编译 [divby0.go](https://github.com/Unknwon/the-way-to-go_ZH_CN/tree/master/eBook/exercises/chapter_4/divby0.go)
...
func main() {
    // 声明一个Rope类型的变量并初始化
    myRope := Rope("Hello, World!")
    
    // 打印变量
    fmt.Println("The rope contains:", myRope)
}
```
...
+ 由于该编码对占用字节长度的不定性，Go 中的字符串也可能根据需要占用 1 至 4 个字节（示例见第 4.6 节），这与其它语言如 C++、Java 或者 Python 不同（Java 始终使用 2 个字节）。
...
1. 在第 7 章，我们会讲到通过将字符串看作是字节（byte）的切片（slice）来实现对其标准索引法的操作。
2. 会在第 5.4.1 节中讲到的 for 循环只会根据索引返回字符串中的纯字节
3. 在第 5.4.4 节（以及第 7.6.1 节的示例）将会展示如何使用 for-range 循环来实现对 Unicode 字符串的迭代操作。
4. 在下一节，我们会学习到许多有关字符串操作的函数和方法，同时 fmt 包中的 fmt.Sprint(x) 也可以格式化生成并返回你所需要的字符串（第 4.4.3 节）。

**练习 4.6** [count_characters.go](https://github.com/Unknwon/the-way-to-go_ZH_CN/tree/master/eBook/exercises/chapter_4/count_characters.go)
...
- 对于`str1`，它只包含ASCII字符（英文字母和空格），每个字符占用1个字节。因此，`len(str1)`和`utf8.RuneCountInString(str1)`的值相同，都是22。
- 对于`str2`，它包含了ASCII字符和日文字符。日文字符在UTF-8编码中通常占用3个字节。因此，`len(str2)`返回的是字符串的总字节数，即28个字节。而`utf8.RuneCountInString(str2)`返回的是字符串中的字符数，包括日文字符，因此是24个字符。

**总结**

- `len`函数给出的是字符串的字节长度，不考虑字符的实际数量。
- `utf8.RuneCountInString`函数给出的是字符串中的Unicode字符数量，正确处理多字节字符。

这就是为什么`str2`的字节数和字符数不同的原因：日文字符占用了额外的字节，但在字符计数时，每个日文字符只算作一个字符。
...
示例 4.13 [presuffix.go](https://learnku.com/docs/the-way-to-go/strings-and-strconv-packages/examples/chapter_4/presuffix.go)
...
示例 4.14 [index_in_string.go](https://learnku.com/docs/the-way-to-go/strings-and-strconv-packages/examples/chapter_4/index_in_string.go)
...
示例 4.15 [count_substring.go](https://learnku.com/docs/the-way-to-go/strings-and-strconv-packages/examples/chapter_4/count_substring.go)
...
示例 4.16 [repeat_string.go](https://learnku.com/docs/the-way-to-go/strings-and-strconv-packages/examples/chapter_4/repeat_string.go)
...
示例 4.19 [string_conversion.go](https://learnku.com/docs/the-way-to-go/strings-and-strconv-packages/examples/chapter_4/string_conversion.go)
...
示例 4.20 [time.go](https://learnku.com/docs/the-way-to-go/time-and-date/examples/chapter_4/time.go)
...
![file](./assets/0-20241130175107875.png)

程序 string_pointer.go 为我们展示了指针对 string 的例子。

**它展示了分配一个新的值给 *p 并且更改这个变量自己的值（这里是一个字符串）。**

示例 4.22 [string_pointer.go](https://learnku.com/docs/the-way-to-go/time-and-date/examples/chapter_4/string_pointer.go)
...
![file](./assets/0-20241130175314247.png)
...
所以说，Go 语言和 C、C++ 以及 D 语言这些低层（系统）语言一样，都有指针的概念。但是对于经常导致 C 语言内存泄漏继而程序崩溃的指针运算
...
1. 指针传递是很廉价的，只占用 4 个或 8 个字节。当程序在工作中需要占用大量的内存，或很多变量，或者两者都有，使用指针会减少内存占用和提高效率。
2. 被指向的变量也保存在内存中，直到没有任何指针指向它们，所以从它们被创建开始就具有相互独立的生命周期。

缺点：

1. 一个指针导致的间接引用（一个进程执行了另一个地址），指针的过度频繁使用也会导致性能下降。
2. 指针也可以指向另一个指针，并且可以进行任意深度的嵌套，导致你可以有多级的间接引用，但在大多数情况这会使你的代码结构不清晰。

如我们所见，在大多数情况下 Go 语言可以使程序员轻松创建指针，并且隐藏间接引用，如：自动反向引用。

**对一个空指针的反向引用是不合法的，并且会使程序崩溃：**

示例 4.23 [testcrash.go](https://learnku.com/docs/the-way-to-go/time-and-date/examples/chapter_4/testcrash.go):
...
3. **函数参数传递**：
   ...
   *i = 10
   ...
   var i int = 5
   modifyInt(&i) // i 现在是 10
   ```

   在函数参数中使用`*`表示该参数是一个指针，允许函数修改原始变量的值。
...
4. **接收者类型为指针的接口**：		
   ...
   在接口方法声明中，如果接收	者是指针类型，可以使用`*`号。

5. **Map 元素赋值**（Go 1.8引入的语法糖）：
   ...
   *m["key"] = 42
   ...
   这等同于`m["key"] = 42`，`*`号用于直接在解引用的map元素上赋值。